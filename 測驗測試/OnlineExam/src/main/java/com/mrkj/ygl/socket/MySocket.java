...
public class MySocket {
    ...
    public void onOpen(Session session,EndpointConfig config) throws IOException{
        ...
        String username = (String)userInfoMap.get(UserInfo.username.toString());
    ...
    public void onMessage(String message, Session session) {
        ...
        if (message != null){
			...
			case "start":// 从这步开始考试正式开始
				//httpSession中存放了当前试卷的时间
				String time = (String)httpSession.getAttribute(BusinessMain.answertime.toString());
				//计算答题时间，单位为分钟
				...
				//开始考试 ，倒计时开始时，会向客户端发送一个start状态标记，客户端使用JavaScript同步计时开始。
				//计时结束，会向客户端发送一个end状态标记，客户端使用JavaScript提交表单，完成一次考试
				Timekeeping timekeeping = new Timekeeping(target,session);
				Thread thread = new Thread(timekeeping);
				//启动线程
				thread.start();
				break;
			case "close": 	// 用户关闭了浏览器，会向服务器发送一条数据，这里可做持久化算分处理	
