...
public class ActionQuestionServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if ("action".equals(action)) {
			...
			session.setAttribute(BusinessMain.answertime.toString(), questions.get(BusinessMain.answertime.toString()));
		...
		} else if ("kfcx".equals(action)) {	//前台成绩查询
			HttpSession session = req.getSession();
			...
			String HtmlPage = Util.page("action?act=kfcx" , page, lastPage);
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		HttpSession session = req.getSession();
		String username = (String) session.getAttribute(UserInfo.username.toString()); // 获取用户名

		if (BusinessService.countScore(parmMap, username) == 1) {  // 计算分数
			resp.sendRedirect("action?act=kfcx");  // 显示成绩
		} else {
