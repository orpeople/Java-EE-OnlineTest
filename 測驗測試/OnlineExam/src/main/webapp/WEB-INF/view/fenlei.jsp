	...
	<div class="container">
		<table>
			<tr>
				...
				<td >
					<div class="text-center" style="padding-bottom: 10px;padding-top: 30px;color:#FFFFFF">
						您好,尊敬的会员:${USERNAME }
					</div>
				</td>
			</tr>
...
<div class="container" >
	...
	<table class="table table-bordered " style="border-spacing: 30px;" >
		<!--  c:choose与c:when是成组出现，想要使用c:when就必须把它写在c:choose中 -->
		<c:choose>
			<%--使用c:when判断数据类型是不是空，避免空指针异常 --%>
			<c:when test="${not empty subjects }">
				<!--使用c:forEach迭代科目数据 -->
				<c:forEach items="${subjects }" var="subject" varStatus="vs">
					<!--从这里开始把数据模型数据写入到HTML中 -->
					<c:if test="${vs.index % 4 == 0 }">
						<c:set var="flagIndex" value="${vs.index + 4}"></c:set>
						<tr>
					</c:if>
						<td >
						<!--获取科目内容，并制作一个连接，为了获取科目下的试卷 -->
...
<div style="background-color: #293448;position: fixed;bottom: 0;width: 100%;">
	<div class="container-fluid text-center" style="height: 80px;line-height: 80px;" >
		<span style="color: #ffffff;">吉林省明日科技有限公司 Copyright &copy; <a href="www.mingrisoft.com">www.mingrisoft.com</a> AllRights Reserved 备案号:吉ICP证07500273</span>
