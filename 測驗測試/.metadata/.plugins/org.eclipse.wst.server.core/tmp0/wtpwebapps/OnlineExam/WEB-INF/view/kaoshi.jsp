...
<body onkeydown="keydown()">
	<div class="container-fluid" style="background-color: #293448;">
		<div class="container">
			<table>
				<tr>
					<td style="width: 160px; padding-left: 80px;"><img alt=""
						src="<%=basePath%>imgs2/logo.png"></td>
					...
					<td>
						<div class="text-center"
							style="padding-bottom: 10px; padding-top: 30px; color: #FFFFFF">
							你好,尊敬的会员:${USERNAME }</div>
					</td>
				</tr>
			</table>
		</div>
		...
		<form id="actionSub" action="<%=basePath%>action" method="post">
			<h3 class="text-primary text-center">
				<span class="glyphicon glyphicon-tags"></span><span
					style="padding-left: 9px;">${questions.BIAOTI}</span>
			</h3>
			<div class="well text-primary">
				<span class="glyphicon glyphicon-book">试卷说明：</span> <br />
				<ol>
					<li>作答时间为 ${DATISHIJIAN} 分钟,总分${questions.totalScore}分.</li>
						...
						<tbody>
							<c:forEach items="${questions.questions }" var="question"
								...
								<tr class="success" ondblclick="setFlag(${vs.index })">
									<th><a name="a${vs.index }" id="a${vs.index }"
										style="padding-top: 61px;"></a>
										<h4 style="color: #000000;">
											<span class="glyphicon glyphicon-paperclip text-danger"></span>${vs.index + 1 }.${question.WENTI }[<a
												href="javascript:void(0)" onclick="setFlag(${vs.index })">答题标记</a>]
										</h4></th>
								</tr>
								<tr>
									<c:choose>
												...
												<ol class="text-primary" style="font-size: 14px;">
													<c:forEach items="${questions.answers}" var="answer"
														...
														<c:if test="${question.que_id eq answer.que_id }">
															<c:if test="${question.LEIXING eq 'radio'}">
																<%
																	//这里是单选
																%>
																<li>
																	<div class="radio">
																		<label><input type="radio"
																			value="${answer.ans_id }" name="${answer.que_id }"
																			aria-label="...">${answer.DAAN}</label>
																	</div>
																</li>
															</c:if>
															<c:if test="${question.LEIXING eq 'checkbox'}">
																<%
																	//这里是多选
																%>
																<li>
										...
										<c:otherwise>
											<td style="padding-left: 40px"><span>暂无任何答案</span></td>
										</c:otherwise>
									</c:choose>
								</tr>
							</c:forEach>
	...
	<div class="rn-guild">
		<div class="panel panel-success">
			<div class="panel-heading">考试剩余时间</div>
			<div class="panel-body">
				<a href="#">倒计时 <span id="t_h"></span>时<span id="t_m"></span>分<span
					id="t_s"></span>秒
				</a>
			</div>
		</div>
				...
				<c:choose>
					<c:when test="${not empty questions.questions }">
						<table class="table table-bordered">
							<c:forEach items="${questions.questions }" var="items"
								varStatus="vs">
								<c:if test="${vs.index % 6 == 0 }">
									<c:set var="flagIndex" value="${vs.index + 6}"></c:set>
									<tr>
								</c:if>
								<td><a id="dtk${vs.index }" class="btn btn-info btn-xs"
									href="javascript:void(0);" onclick="goIndex(${vs.index})">
										<c:if test="${(vs.index+1)<10 }">
    								0${vs.index+1}
    							</c:if> <c:if test="${(vs.index+1)>=10 }">
    								${vs.index+1}
    							</c:if>
								</a></td>
			...
			<div class="panel-footer">
				<p class="text-center">
					<button type="button" class="btn btn-success btn-sm"
						onclick="submitGo();">
						<span class="glyphicon glyphicon-pencil"></span>交卷
					</button>
				</p>
			</div>
		</div>
	...
	<div
		style="background-color: #293448; position: fixed; bottom: 0; width: 100%;"
		id="footer">
		<div class="container-fluid text-center"
			style="height: 80px; line-height: 80px;">
			<span style="color: #ffffff;">吉林省明日科技有限公司 Copyright &copy;<a
				href="www.mingrisoft.com">www.mingrisoft.com</a> AllRights Reserved
				备案号:吉ICP证07500273
			</span> <span><a href="javascript:void(0);" onclick="hideFooter();">隐藏</a></span>
		</div>
	</div>
	...
	<script type="text/javascript"
		src="<%=basePath%>js/websocket.js"></script>
	<script type="text/javascript">
